...
public class Grid {
    ...
    public void deployShip(Ship s) { // INSERT YOUR CODE HERE
        ...
        while ( !empty ) {
            ...
            if (emptyCount == shipSize) {empty = true; }
        ...
        if (alignment == 0) {
             for (int i = 0; i < shipSize; i++) {
                  Square a = new ShipSquare(s, i);
                  grid.get(row).set(col + i, a);
            }
        }
        else {
            for (int i = 0; i < shipSize; i++) {
            Square a = new ShipSquare(s, i);    
            grid.get(row + i).set(col, a);
            }
        }
